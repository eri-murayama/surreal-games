// ========================================
// Golden Gold Driver ~A Country Boy's Challenge~
...
(function () {
  ...
  // ===== High Score =====
  const HS_KEY = 'itch_golden_driver_highscore';
  ...
  const SLOT_ITEMS = {
    body: [
      { emoji: '🏎️', name: 'Sports Car', stat: 'speed', value: 2 },
      { emoji: '🚗', name: 'Sedan', stat: 'speed', value: 1 },
      { emoji: '🛻', name: 'Truck', stat: 'speed', value: 0 },
      { emoji: '🚜', name: 'Tractor', stat: 'speed', value: -1 },
      { emoji: '🛒', name: 'Shopping Cart', stat: 'speed', value: -2 },
    ],
    engine: [
      { emoji: '🔥', name: 'Turbo Engine', stat: 'accel', value: 2 },
      { emoji: '⚡', name: 'Electric Motor', stat: 'accel', value: 1 },
      { emoji: '💨', name: 'Gas Engine', stat: 'accel', value: 0 },
      { emoji: '🐹', name: 'Hamster Power', stat: 'accel', value: -1 },
      { emoji: '🧠', name: 'Psychic Engine', stat: 'accel', value: -2 },
    ],
    tire: [
      { emoji: '⭕', name: 'Racing Tires', stat: 'handling', value: 2 },
      { emoji: '🟤', name: 'Normal Tires', stat: 'handling', value: 1 },
      { emoji: '🍩', name: 'Donuts', stat: 'handling', value: 0 },
      { emoji: '🍊', name: 'Oranges', stat: 'handling', value: -1 },
      { emoji: '👞', name: "Prof's Shoes", stat: 'handling', value: -2 },
    ],
  };

  // Items
  const RACE_ITEMS = [
    { name: 'Banana', emoji: '🍌', effect: 'banana' },
    { name: 'Dash', emoji: '🚀', effect: 'boost' },
    { name: 'Onigiri', emoji: '🍙', effect: 'shield' },
  ];

  // Rival names
  const RIVAL_NAMES = ['Gantetsu', 'Hiroshi', 'Masao'];
  ...
  function updateHighScoreDisplay() {
    ...
    if (hs !== null) {
      titleHighscore.textContent = `👑 Best Time: ${hs}s`;
  ...
  function initSlots() {
    ...
    hakaseSpeech.textContent = '"Let\'s pick parts with the slot!"';
  ...
  function spinAllReels() {
    ...
    slotBtn.disabled = false;
    hakaseSpeech.textContent = '"Click or press Space to stop!"';
  }
  ...
  function stopNextSlot() {
    ...
    slotStep++;
    if (slotStep < 3) {
      const partLabels = ['Engine', 'Tire'];
      hakaseSpeech.textContent = `"Now stop the ${partLabels[slotStep - 1]}!"`;
    } else {
      slotSpinning = false;
      onAllSlotsComplete();
    }
  ...
  function onAllSlotsComplete() {
    ...
    setTimeout(() => {
      ...
      slotResult.innerHTML = `Body: ${b}<br>Engine: ${e}<br>Tire: ${ti}` +
        `<div style="margin-top:6px;">Prof: "Not bad at all!"</div><div>Yoshinori: "..."</div>`;

      raceBtn.textContent = 'Next';
  ...
  function raceLoop() {
    ...
    hudPosition.textContent = 'Pos: ' + posLabels[position - 1];
  ...
  function useItem(p) {
    ...
    switch (item.effect) {
      case 'banana':
        ...
        showItemEffect('🍌 Banana dropped!', '🍌', '#ffee00');
        SoundSystem.play('banana_set');
        break;
      case 'boost':
        p.boostTimer = 60;
        showItemEffect('🚀 DASH!!', '🚀', '#ff4400');
        SoundSystem.play('boost');
        break;
      case 'shield':
        p.shielded = true;
        p.shieldTimer = 180;
        showItemEffect('🍙 Onigiri Barrier!', '🍙', '#66ccff');
  ...
  function drawRace() {
    ...
    if (lapEffect.active) {
      ...
      const subTxt = lapEffect.lap === 0
        ? ''
        : lapEffect.lap === LAPS ? 'Final Lap!' : 'Nice Run!';
      ctx.fillStyle = '#fff';
  ...
  function endRace() {
    ...
    lastResultSec = sec;
    resultTitleEl.textContent = `Result: ${posLabels[position - 1]}!`;
    resultTitleEl.style.color = position === 1 ? '#ffd700' : '#cc6600';
    resultComment.textContent = RESULT_COMMENTS[position - 1];
    resultTime.textContent = `Time: ${sec}s`;

    // High score (1st place only, lower is better)
    let isNewHigh = false;
    if (position === 1) {
    ...
    const hs = getHighScore();
    if (hs !== null) {
      resultHighscore.textContent = `👑 Best Time: ${hs}s`;
  ...
  shareBtn.addEventListener('click', () => {
    const shareText = `🏎️ Golden Gold Driver\nResult: ${lastResultPos}\nTime: ${lastResultSec}s\n\nPlay: https://surrealgames.itch.io/golden-gold-driver\n#GoldenDriver`;
