...
function gameClear() {
  ...
  overlay.innerHTML = `
    ...
    <div class="clear-buttons">
      <button class="clear-back-link clear-play-again">${t('clearPlayAgain')}</button>
    </div>
  `;
  document.body.appendChild(overlay);

  overlay.querySelector('.clear-play-again').addEventListener('click', resetGame);

  createConfetti();
  createParticles();
}

// ===== ゲームリセット（Plicy iframe対応） =====
function resetGame() {
  // クリア演出を削除
  ...
  state.inventory = [];
  state.selectedItem = null;
  state.memoPickedUp = false;
  state.flashlightPickedUp = false;
  state.dustPickedUp = false;
  state.bentoPickedUp = false;
  state.safeWrongCount = 0;
  state.safeBroken = false;
  state.memoRead = false;
  state.paintingDestroyed = false;
  state.doorUnlocked = false;
  state.cleared = false;
  state.hiddenRevealed = false;
  state.windowPeeked = false;
  state.startTime = Date.now();
  Object.keys(state.checkedItems).forEach(k => state.checkedItems[k] = false);

  // DOM リセット
  door.classList.remove('door-unlocked', 'door-open');
  painting.classList.remove('painting-destroyed');
  safe.classList.remove('safe-broken');
  hiddenMessage.classList.remove('revealed');
  memoOnFloor.classList.remove('hidden');
  flashlightOnFloor.classList.remove('hidden');
  dustOnFloor.classList.remove('hidden');
  bentoOnFloor.classList.remove('hidden');

  // 金庫モーダルリセット
  safeCode = '';
  document.querySelectorAll('.code-digit').forEach(d => d.textContent = '_');
  document.getElementById('safe-result').textContent = '';
  document.getElementById('safe-smash-btn').classList.add('hidden');
  safeDisplay.textContent = '---';

  // モーダル全閉じ
  [safeModal, memoModal, windowModal, paintingModal, itemInspectModal].forEach(m => m.classList.add('hidden'));

  // アイテムボックスリセット
  renderInventory();

  // セリフリセット
  dialogText.innerHTML = t('opening');
  dialogName.textContent = t('protagonist');

  // 窓セリフリセット
  windowLineIndex = 0;
  if (windowSpeechTimer) {
    clearInterval(windowSpeechTimer);
    windowSpeechTimer = null;
  }
  windowSpeech.classList.remove('visible');
  setTimeout(startWindowSpeech, 5000);
}
