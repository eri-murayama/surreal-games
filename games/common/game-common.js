...
(function () {
  ...
  const BGM_PRESETS = {
    ...
    // パリ・ファッション風（カブトムシ様のお着換え）— ミュゼット／シャンソンのおしゃれワルツ
    // Dm基調の半音階下降と E7 への寄り道で「シャンゼリゼ感」を演出。
    // sawtooth でアコーディオンのリードっぽい鼻にかかった倍音を出す。
    paris: {
      tempo: 170, key: 'Dm', wave: 'sine', volume: 0.09,
      melody: [
        // ===== 前半（Aパート・Dm）しっとりミュゼット =====
        // M1: タタターン×2「ラ・ソ・ファーン  ミ・レ・ドーン」
        440, 392, 349,   0, 330, 294, 277,   0,
        // M2: ターン・ターン・タタタターン「ファーン ミーン  レ・ファ・ラ・ソーン」
        349,   0, 330,   0, 294, 349, 440, 392,
        // M3: タタ・タタ・ターン×2 でフレーズ反復
        349, 392, 440, 466, 440,   0, 349,   0,
        // M4: 着地「ファ・ミ・レ・ドーン  レーン」
        349, 330, 294,   0, 294,   0, 220,   0,
        // ===== 後半（Bパート・F majorに転調＋加速）軽やかに弾む =====
        // M5: タタタタ・ターン「ド・レ・ミ・ファーン  ミ・レ・ドーン」
        523, 587, 659, 698,   0, 587, 523, 440,
        // M6: タタタターン「ド・ミ・ソ・シ♭ーン」上昇のフック
        523, 659, 784, 932,   0, 784, 698, 587,
        // M7: 下降「ミ・レ・ドーン  ラ・ソ・ファーン」
        659, 587, 523,   0, 440, 392, 349,   0,
        // M8: Dm に戻る橋渡し「ラ・ソ・ファ・ミーン  レーン」
        440, 392, 349, 330,   0, 294,   0, 294,
      ],
      bass: [
        // 前半: D-A walking（リズムに合わせて点在）
         73,  73, 110,   0, 110, 110,  73,   0,
         73,   0, 110,   0, 110,  73,  87, 110,
         87, 110,  73, 110, 110,   0,  87,   0,
         87,  73, 110,   0, 110,   0,  73,   0,
        // 後半: F-C walking
         87, 130,  98, 130,   0, 130,  87, 130,
         87, 130,  98, 130,   0, 130,  87, 130,
        110, 130,  87,   0, 110, 130,  87,   0,
         98, 130,  87, 110,   0, 110,   0,  73,
      ],
      // タタタターン感を出す swing（短-短-短-長、長-長-短-短 など）
      swing: [
        // M1: 0.6 0.6 1.2 0.6  0.6 0.6 1.2 0.6 = 6.0
        0.6, 0.6, 1.2, 0.6, 0.6, 0.6, 1.2, 0.6,
        // M2: 1.2 0.5 1.2 0.5  0.5 0.5 0.5 1.5 = 6.4
        1.2, 0.5, 1.2, 0.5, 0.5, 0.5, 0.5, 1.5,
        // M3: 0.5 0.5 0.5 1.5  0.5 0.5 1.5 0.5 = 6.0
        0.5, 0.5, 0.5, 1.5, 0.5, 0.5, 1.5, 0.5,
        // M4: 0.6 0.6 0.6 1.2  0.6 0.6 1.4 0.6 = 6.2
        0.6, 0.6, 0.6, 1.2, 0.6, 0.6, 1.4, 0.6,
        // M5: 0.5 0.5 0.5 1.5  0.5 0.5 0.5 1.5 = 6.0
        0.5, 0.5, 0.5, 1.5, 0.5, 0.5, 0.5, 1.5,
        // M6: 0.5 0.5 0.5 1.5  0.5 0.5 0.5 1.5 = 6.0
        0.5, 0.5, 0.5, 1.5, 0.5, 0.5, 0.5, 1.5,
        // M7: 0.5 0.5 1.4 0.6  0.5 0.5 1.4 0.6 = 6.0
        0.5, 0.5, 1.4, 0.6, 0.5, 0.5, 1.4, 0.6,
        // M8: 0.6 0.6 0.6 0.8  0.8 1.0 1.0 1.6 = 7.0
        0.6, 0.6, 0.6, 0.8, 0.8, 1.0, 1.0, 1.6,
      ]
    },
    ...
    cafe: {
      ...
      melody: [
        ...
        466, 523, 587, 659, 587, 523, 466, 440,
        440, 523, 587, 698, 659, 587, 523, 440,
      ],
      bass: [
        175, 0, 131, 0, 175, 0, 131, 0,
        117, 0, 175, 0, 117, 0, 175, 0,
        196, 0, 147, 0, 196, 0, 147, 0,
        131, 0, 196, 0, 131, 0, 175, 0,
      ],
      // ボサノバのスウィング（ゆったりした長短リズム）
      swing: [
        1.3, 0.7, 1.3, 0.7, 1.2, 0.8, 1.3, 0.7,
        1.2, 0.8, 1.3, 0.7, 1.2, 0.8, 1.3, 0.7,
        1.3, 0.7, 1.2, 0.8, 1.3, 0.7, 1.2, 0.8,
        1.2, 0.8, 1.3, 0.7, 1.2, 0.8, 1.3, 0.7,
      ]
    },
    // タイピング系（シュールタイピング）— タイプライターのような小気味よい連打リズム
    // tap-tap-tap-TAP の繰り返しで「指がキーを叩く」感を演出
    typing: {
      tempo: 145, key: 'D', wave: 'square', volume: 0.07,
      melody: [
        587, 880, 587, 1175, 880, 587, 740, 988,
        740, 1175, 880, 587, 988, 740, 880, 1319,
        494, 740, 587, 988, 740, 494, 587, 880,
        659, 988, 740, 1175, 880, 740, 587, 494,
      ],
      bass: [
        147, 220, 147, 220, 196, 247, 196, 247,
        147, 220, 147, 220, 196, 247, 147, 220,
        123, 196, 123, 196, 147, 220, 147, 220,
        165, 247, 165, 247, 196, 247, 147, 220,
      ],
      // 短-短-短-長 の連打スウィング（タイピング感）
      swing: [
    ...
    battle: {
      ...
      melody: [
        ...
        440, 0, 659, 0, 880, 0, 659, 440,
        494, 740, 0, 988, 880, 740, 587, 0,
      ],
      bass: [
        247, 370, 247, 370, 247, 330, 247, 330,
        294, 370, 294, 370, 247, 330, 247, 370,
        220, 330, 220, 330, 247, 370, 247, 370,
        247, 370, 294, 440, 330, 370, 247, 247,
      ],
      brass: [
        494, 0, 0, 494, 0, 0, 740, 0,
        494, 0, 0, 0, 0, 740, 0, 494,
        440, 0, 0, 440, 0, 0, 659, 0,
        494, 0, 740, 0, 0, 0, 494, 494,
      ],
      swing: [
        0.6, 1.4, 0.6, 1.4, 0.6, 1.4, 0.6, 1.4,
        0.7, 1.3, 0.6, 1.4, 0.7, 1.3, 0.6, 1.4,
        0.6, 1.4, 0.6, 1.4, 0.7, 1.3, 0.6, 1.4,
        0.7, 1.3, 0.6, 1.4, 0.6, 1.4, 0.7, 1.3,
      ]
    },
  };
